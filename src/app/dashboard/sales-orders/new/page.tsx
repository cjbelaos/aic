...
export default function NewSalesOrderPage(): React.ReactNode {
  ...
  const createFromQuotation = async (quotationNo: string): Promise<void> => {
    ...
    router.push(result.order.reusedExisting ? `/dashboard/sales-orders/${id}` : `/dashboard/sales-orders/${id}/edit`);
  };
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1600px] min-w-0 flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-semibold tracking-tight">Create Sales Order</h1><p className="text-sm text-muted-foreground">Enter the customer, commercial details, products or services, and pricing.</p></div>
        <Button variant="outline" onClick={() => router.push("/dashboard/sales-orders")}><ArrowLeft className="mr-2 h-4 w-4" />Back to Sales Orders</Button>
      </div>
      <OrderForm
