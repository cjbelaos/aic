...
export default function EditSalesOrderPage(): React.ReactNode {
  ...
  const [detail, setDetail] = React.useState<Awaited<
    ReturnType<typeof salesOrderService.get>
  > | null>(null);
  ...
  React.useEffect(() => {
    Promise.all([salesOrderService.options(), salesOrderService.get(orderId)])
      .then(([optionResult, detailResult]) => {
      ...
      .catch((caught) =>
        setError(
          caught instanceof Error
            ? caught.message
            : "Failed to load the Sales Order.",
        ),
      );
  }, [orderId]);
  ...
  if (!options || !detail) return <PageLoader label="Loading sales order" />;
