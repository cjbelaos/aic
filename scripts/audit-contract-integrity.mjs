...
export function auditContracts(contractRows, legacyRows, v2Rows, releaseRows) {
  ...
  const children = [{ sheet: "ContractItems", rows: legacyRows, contractColumn: 1 }, { sheet: "ContractItemsV2", rows: v2Rows, contractColumn: 1 }, { sheet: "ContractReleases", rows: releaseRows, contractColumn: 2 }];
...
if (process.argv.includes("--self-test")) {
  ...
  const report = auditContracts([["CTR-0001"], ["CTR-0003"], ["CTR-0003"], [""]], [["CTI-1", "CTR-0001"], ["CTI-2", "CTR-9999"]], [["CTI-3", "CTR-8888"]], [["REL-1", "CTI-1", "CTR-7777"]]);
  assert.equal(report.issueCount, 5);
  checks += 4; // duplicate parent plus three orphan stores
...
function credentials() {
  ...
  if (raw) { const parsed = JSON.parse(raw); return { email: parsed.client_email, key: parsed.private_key.replace(/\\n/g, "\n") }; }
  const keyFile = process.env.GOOGLE_SERVICE_ACCOUNT_KEY_FILE;
  if (keyFile && existsSync(keyFile)) { const parsed = JSON.parse(readFileSync(keyFile, "utf8")); return { email: parsed.client_email, key: parsed.private_key.replace(/\\n/g, "\n") }; }
...
const [contracts, legacy, v2, releases] = await Promise.all([read("Contracts!A2:I"), read("ContractItems!A2:F"), read("ContractItemsV2!A2:G"), read("ContractReleases!A2:M")]);
console.log(JSON.stringify(auditContracts(contracts, legacy, v2, releases), null, 2));
